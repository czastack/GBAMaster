...
public class CotResource {
	...
	private ZipFile mCotZip; //模板资源在zip压缩包(.cot)中
	private File mCotDir; //模板资源在目录中(优先查找)
	...
	public CotResource(String path) throws Exception{
		File guessDir = new File(path/*.substring(0, path.lastIndexOf('.'))*/);
		if (guessDir.isDirectory()){
			mCotDir = guessDir;
		} else {
			mCotZip = new ZipFile(path);
		}
	...
	public InputStream open(String entryName) throws Exception{
		if (mCotDir != null) {
			return new FileInputStream(new File(mCotDir, entryName));
		}
		else {
			ZipEntry entry = mCotZip.getEntry(entryName);
			if (entry != null)
				return mCotZip.getInputStream(entry);
			return null;
		}
	}
